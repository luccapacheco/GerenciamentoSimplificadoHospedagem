...
class SistemaGerenciamentoSupabaseUser extends BaseAuthUser {
  SistemaGerenciamentoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> sistemaGerenciamentoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SistemaGerenciamentoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
